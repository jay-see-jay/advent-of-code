...
class PaperRollMap:
    ...
    def clear_rolls(self):
        for r in range(len(self.grid)):
            for c in range(len(self.grid[r])):
                contents = self.grid[r][c]
                if contents == "x":
                    self.grid[r][c] = "."

    def count_all_accessible(self):
        total_count = 0

        while True:
            count = self.count_accessible()
            if count == 0:
                break
            self.clear_rolls()
...
def process_input(logger: Logger, is_test=False, should_repeat=False):
    ...
    if should_repeat:
        answer = map.count_all_accessible()
    else:
...
def handle42(logger: Logger, is_test=False):
    process_input(logger, is_test, True)
